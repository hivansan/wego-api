 ...
 */
export const respond = (fn: RequestFn) => ((req: Request, res: Response) => {
  const handlerResult = fn(req);
  const perf = ['true', '1'].includes('' + req.query.perf);
  ...
  (isPromise(handlerResult) ? handlerResult : Promise.resolve(handlerResult)).then((result: ResponseVal) => {
    ...
    res.set(mergeRight(result.headers || {}, perf ? { Took: Date.now() - start } : {}));
