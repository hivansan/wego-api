 ...
 * Example usage:
 * `./node_modules/.bin/ts-node ./scraper/scraper.assets.ts --exec=saveAssetsFromCollections --bots=4 --errsToFile=./data/errors-to-assets.txt`
 *
 * for just a collection
 * `./node_modules/.bin/ts-node ./scraper/scraper.assets.ts --exec=saveAssetsFromCollections --collectionFilter=nfh --bots=1`
 *
 * from a file
 ...
 */
 ...
 import { map, pick, pipe, toString, prop, props, sortBy, tap, flatten, dropRepeats, split, forEach, filter, mergeRight, path } from 'ramda';
 ...
 const dum = (data: unknown) : any => new Promise((resolve, reject) => { resolve(data) });
